...
func LoadConfig() (*AppEnvConfig, error) {
	serverCfg := &ServerConfig{
		...
		Name:          getEnv("SERVER_NAME", "tianfu-transaction-service"),
	...
	otelCfg := &OTelConfig{
		...
		ServiceName: getEnv("OTEL_SERVICE_NAME", "tianfu-transaction-service"),
	}
...
func (c *AppEnvConfig) GetDBDSN() string {
	return fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s sslmode=%s TimeZone=%s",
		c.dbCfg.Host, c.dbCfg.Port, c.dbCfg.User, c.dbCfg.Password,
		c.dbCfg.Name, c.dbCfg.SSLMode, c.dbCfg.Timezone)
}
